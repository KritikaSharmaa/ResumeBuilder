...
const CodeA=()=>{
    ...
    return <div className="h-screen w-full bg-pink-700 text-white              font-semibold flex flex-col p-2">
            <div className="w-full h-auto bg-green-500 flex">
                <img src="http://simpleicon.com/wp-content/uploads/add-user.svg" className="h-28 w-28 rounded-3xl"></img>
                <div className="h-auto w-11/12">
                    <h3 className="h-1/5 w-full bg-blue-400 text-center mb-2">{personalDetails.Fname} {personalDetails.Lname}</h3>
                    <div className="h-auto w-full text-center">{personalDetails.ProfSum}</div>
                    <div className="flex flex-wrap justify-evenly pl-4 pr-4 pt-3 h-4/5 w-full bg-blue-400 text-center">
                        <p className="ml-2 mb-0">{personalDetails.Email}</p>
                        <p className="ml-2 mb-0">{personalDetails.City}</p>
                        <p className="ml-2 mb-0">{personalDetails.State}</p>
                        <p className="ml-2 mb-0">{personalDetails.Phone}</p>
            ...
            <div className="w-full max-h-20 bg-green-400 flex">
                <div className="h-full w-3/12 bg-blue-400">Skills</div>
                <ul className="flex flex-wrap h-full w-full bg-yellow-500">
                {
                    Skills.map(myskill=>{
                        return <li className="h-auto w-1/2 bg-yellow-200">{myskill}</li>
                    })
                }
                </ul>
            </div>
            <div className="w-full max-h-72 bg-gray-100 flex">
                <div className="h-full w-3/12 bg-blue-400 pt-2">Education</div>
                <div className="flex flex-col h-full w-full bg-yellow-500 mb-0">
                    <div className="h-full w-full mt-2">
            ...
            <div className="w-full h-auto bg-green-400 flex">
                <div className="h-full w-3/12 bg-blue-400">Projects</div>
                <div className="flex flex-col h-full w-full bg-pink-500 mb-0">
                    {
                        Projects.map(myprojects=>{
                            return <div className="h-auto w-full mt-2">
                                    <div className="tracking-widest leading-tight mb-0">&#x2713;{myprojects.Title}</div>
            ...
            <div className="w-full h-auto bg-green-400 flex">
                <div className="h-full w-3/12 bg-blue-400 pt-2">Achievements</div>
                <ul className="flex flex-col h-full w-full bg-pink-500 mb-0 pt-2">
                {
                    Achievements.map(myAch=>{
                        return <li>{myAch}</li>
                    })
